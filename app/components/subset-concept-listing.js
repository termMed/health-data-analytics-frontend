        ...
        autoComplete(param) {
            var run = function(scope) {
                    if(param !== "" && param !== '*' && scope.get('mrcmType') === null && (scope.get('typeId') === null || scope.get('typeId') === '*')) {
                            ...
                            scope.get('ajax').request('/mrcm/MAIN/attribute-values/'+scope.get('typeId')+'?termPrefix='+ param + '*&expand=fsn()&offset=0&limit=50')
                                .then((result) => {
                                var filteredAttrs = [];
                                result.items.forEach(function(item){
                            ...
                            scope.get('ajax').request('/mrcm/MAIN/domain-attributes?parentIds=' + scope.get('parentId') + '&expand=fsn()&offset=0&limit=50')
                                .then((result) => {
                                var filteredAttrs = [];
                                result.items.forEach(function(item){
                                    item.fsn = item.fsn.term;
